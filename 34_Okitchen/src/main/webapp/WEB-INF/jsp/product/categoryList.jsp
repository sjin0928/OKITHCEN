...
<body>
	<%-- \${categoryProdList } : ${categoryProdList } <hr> --%>
	<%-- \${prodListCnt } : ${prodListCnt } <hr> --%>
	...
	<div class="css-nu1lw1 e12kjs5e0">
		<div id="container" class="css-1b38o8c eakz2kp1">
			...
			<c:choose>
				<c:when test="${prodListCnt == 0}">
					<h3 class="css-i804ml eaf14xt0">등록된 상품이 없습니다.</h3>
				</c:when>
				<c:otherwise>

					<%-- <c:forEach var="category" items="${categoryProdList}">
				<c:choose>
				    <c:when test="${category.categoryName eq 'vege'}">
				        <h3 class="css-i804ml eaf14xt0">채소</h3>
				    </c:when>
				    <c:when test="${category.categoryName eq 'fruit'}">
				        <h3 class="css-i804ml eaf14xt0">과일</h3>
				    </c:when>
				    <c:when test="${category.categoryName eq 'meat'}">
				        <h3 class="css-i804ml eaf14xt0">정육/계란</h3>
				    </c:when>
				    <c:when test="${category.categoryName eq 'seafood'}">
				        <h3 class="css-i804ml eaf14xt0">해산물</h3>
				    </c:when>
				    <c:when test="${category.categoryName eq 'snack'}">
				        <h3 class="css-i804ml eaf14xt0">간식/디저트</h3>
				    </c:when>
				    <c:when test="${category.categoryName eq 'bakery'}">
				        <h3 class="css-i804ml eaf14xt0">베이커리</h3>
				    </c:when>
				    <c:when test="${category.categoryName eq 'seasoning'}">
				        <h3 class="css-i804ml eaf14xt0">조미료</h3>
				    </c:when>
				    <c:when test="${category.categoryName eq 'drink'}">
				        <h3 class="css-i804ml eaf14xt0">생수/음료</h3>
				    </c:when>
				    <c:otherwise>
				        <h3 class="css-i804ml eaf14xt0">간편식/샐러드</h3>
				    </c:otherwise>
				</c:choose>
			</c:forEach> --%>
						...
						<div class="css-1d3w5wq ef36txc6">
							...
							<div class="swiper-slide swiper-slide-active">
								...
								<div class="css-11kh0cw ef36txc5">
									<c:forEach items="${categoryProdList }" var="prod">
