...
<style>
    ...
    #ibb th, #bbs td {
        ...
        padding: 8px 12px; /* 조금 더 여백을 주기 위해 패딩 조정 */
        text-align: center;
    }
               ...
               <div class="row">
                  ...
                  <div class="card shadow col-lg-12">
                     <div class="card-header py-3">
                        <h6 class="m-0 font-weight-bold text-primary">전체 상품 내역 (${cnt }건)</h6>
                     </div>
...
<%--    <h3>전체 상품 내역 (${cnt }건)</h3> --%>
   <table class="table">
        <thead  class="thead">
          <tr class="m-0 font-weight-bold text-primary">
              <th scope="col" class="nowrap">상품번호</th>
		      <th scope="col" class="nowrap">상호</th>
		      <th scope="col" class="nowrap">상품명</th>
		      <th scope="col" class="nowrap">가격</th>
		      <th scope="col" class="nowrap">할인율</th>
		      <th scope="col" class="nowrap">할인가</th>
		      <th scope="col" class="nowrap">재고</th>
		      <th scope="col" class="nowrap">유통기한</th>
		      <th scope="col" class="nowrap">등록일자</th>
		      <th scope="col" class="nowrap">승인여부상태</th>
		      <th scope="col" class="nowrap">상품상태</th>
          </tr>
        </thead>
         ...
         <c:otherwise>
            <c:forEach var="vo" items="${list }">
               <tr>
					...
					<th>${vo.brand }</th>
					<th>
						<a href="${pageContext.request.contextPath }/admin/confirmDetail.do?productId=${vo.productId }">
							${vo.title }
						</a>
					</th>
					<th>${vo.price }</th>
					<th>${vo.discountRate }</th>
					<th>${vo.discountedPrice }</th>
					<th>${vo.stock }</th>
					<th>${vo.expirationDate }</th>
					<th>${vo.manufacturingDate }</th>
					<th>${vo.approvalStatus }</th>
					<th>${vo.productStatus }</th>
				</tr>
            </c:forEach>
         </c:otherwise>
         </c:choose>
      ...
      <tfoot>
         <tr>
            <td colspan="12" style="width: 100%; text-align: center;">
               ...
               <c:if test="${pvo.beginPage != 1 }">
                  <li>
                     <a href="${pageContext.request.contextPath}/admin/allPostList.do&nowPage=${pvo.beginPage - 1 }">이전으로</a>
                  </li>
               </c:if>
               ...
               <c:if test="${pageNo != pvo.nowPage }">
                  <li>
                     <a href="${pageContext.request.contextPath}/admin/allPostList.do&nowPage=${pageNo }">${pageNo }</a>
                  </li>
               </c:if>
               ...
               <c:if test="${pvo.endPage < pvo.totalPage }">
                  <li>
                     <a href="${pageContext.request.contextPath}/admin/allPostList.do&nowPage=${pvo.endPage + 1 }">다음으로</a>
                  </li>
               </c:if>
               </ol>
		...
		<table class="table table-hover">
			  <thead>
			    <tr class="table-active">
			      ...
			      <th scope="col" style="width: 10%;">상호</th>
			      <th scope="col" style="width: 20%;">상품명</th>
			      <th scope="col" style="width: 10%;">가격</th>
			      <th scope="col" style="width: 10%;">할인율</th>
			      <th scope="col" style="width: 10%;">할인가</th>
			      <th scope="col" style="width: 10%;">재고</th>
			      <th scope="col" style="width: 15%;">유통기한</th>
			      <th scope="col" style="width: 15%;">등록일자</th>
			      <th scope="col" style="width: 15%;">승인여부상태</th>
			      <th scope="col" style="width: 15%;">상품상태</th>
			    </tr>
			  </thead>
		  <tbody class="table-group-divider">
		  <c:choose>
	...
	<tfoot>
			<tr>
				<td colspan="4">
					...
					<c:if test="${pvo.beginPage != 1 }">
						<li>
							<a href="/biz/admin/confirmProd.do&nowPage=${pvo.beginPage - 1 }">이전으로</a>
						</li>
					</c:if>
					...
					<c:if test="${pageNo != pvo.nowPage }">
						<li>
							<a href="/biz/admin/confirmProd.do&nowPage=${pageNo }">${pageNo }</a>
						</li>
					</c:if>
					...
					<c:if test="${pvo.endPage < pvo.totalPage }">
						<li>
							<a href="/biz/admin/confirmProd.do&nowPage=${pvo.endPage + 1 }">다음으로</a>
						</li>
					</c:if>
					</ol>
