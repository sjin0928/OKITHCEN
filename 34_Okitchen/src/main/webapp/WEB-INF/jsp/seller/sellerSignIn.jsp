...
<script>
	...
	function passwordCheck(){
		...
		if (!regex.test($("#sellerPassword").val())){
	...
	function registNumCheck(){
 	...
 	function allCheck() {
		...
		if ($("#checkMessage").html() != "사용 가능한 아이디입니다." || $("#checkMessage").html() == "") {
			alert ("아이디 중복 확인을 해주세요.");
		} else if($("#pwCheckMessage").html() != "비밀번호가 일치합니다." || $("#pwCheckMessage").html() == "") {
			alert ("비밀번호 확인 버튼을 클릭해주세요.");
		} else if($("#registCheckMessage").html() != "계속사업자 : 가입 가능합니다." || $("#registCheckMessage").html() == "") {
			alert ("사업자등록번호 확인 버튼을 클릭해주세요.");
		} else if(emailRegex.test($("#customerEmail").val())) {
	...
	<form id="signIn" method="post" action="sellerSignIn.do">
		<div class="container" id="sellerSignInBox">
			<div class="input-group mb-3 SIinput">
				...
				<input type="text" class="form-control SignIninputBox" placeholder="아이디를 입력해주세요(영어+숫자)" id="sellerId" name="sellerId" required>
			...
			<div class="input-group mb-3 SIinput">
				...
				<input type="password" class="form-control SignIninputBox" placeholder="비밀번호를 입력해주세요(영어 +숫자, 8~20자)" id="sellerPassword" name="sellerPassword" required>
			</div>
			<div class="input-group mb-3 SIinput">
				<div class="input-group-prepend">
					<span class="input-group-text signIn-text">&nbsp비밀번호 확인</span>
				</div>
				<input type="password" class="form-control SignIninputBox" placeholder="비밀번호를 확인해주세요" name="confirmPassword" id="confirmPassword" required>
			...
			<input type="submit" class="btn" id="sellerSigninBtn" onclick="allCheck()" value="회원가입">
		</div>
	</form>
	<hr>
