package com.spring.biz.picked.impl;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import com.spring.biz.picked.PickedService;
import com.spring.biz.picked.PickedVO;

@Service("pickedService")
public class PickedServiceImpl implements PickedService{
	@Autowired
	private PickedDAO pickedDAO;
	
	public PickedServiceImpl() {
		System.out.println(">> PickedServiceImpl() 객체 생성");
	}

	// 찜 추가 
	@Override
	public int insertPicked(PickedVO vo) {
		System.out.println(">> PickedServiceImpl.insertPicked vo : " + vo);
		return pickedDAO.insertPicked(vo);
	}

	// 찜 삭제
	@Override
	public int deletePicked(PickedVO vo) {
		System.out.println(">> PickedServiceImpl.deletePicked vo : " + vo);
		return pickedDAO.deletePicked(vo);
	}

	// 찜한 상품 여부 확인(상세페이지로 갈 때 필요)
	@Override
	public PickedVO selectPicked(PickedVO vo) {
		System.out.println(">> PickedServiceImpl.selectPicked vo : " + vo);
		return pickedDAO.selectPicked(vo);
	}

}
